...
Write-Host "Proje yapısı düzenleniyor..."

#----------------------------------------------------
# 1. gui/__init__.py dosyasını oluştur
#----------------------------------------------------
Write-Host "gui\__init__.py dosyası oluşturuluyor..."
Set-Content -Path "gui\__init__.py" -Value "" -Encoding UTF8 -Force
Write-Host "__init__.py dosyası başarıyla oluşturuldu."

#----------------------------------------------------
# 2. run.py dosyasını oluştur
#----------------------------------------------------
Write-Host "run.py dosyası oluşturuluyor..."
$runPythonKodu = @"
import sys
import os
...
if __name__ == "__main__":
    ...
    app.mainloop()
"@
$runPythonKodu | Out-File -FilePath "run.py" -Encoding UTF8 -Force
Write-Host "run.py dosyası başarıyla oluşturuldu."

#----------------------------------------------------
# 3. app.py dosyasını ana menü koduyla güncelle
#----------------------------------------------------
...
if __name__ == "__main__":
    ...
    app.mainloop()
"@
$appPythonKodu | Out-File -FilePath "app.py" -Encoding UTF8 -Force
Write-Host "app.py dosyası ana menü koduyla başarıyla güncellendi."
...
git add .
git commit -m "Proje yapisi duzenlendi: run.py ve __init__.py eklendi, app.py guncellendi"

Write-Host "Değişiklikler GitHub'a gönderiliyor..."
git push origin main

Write-Host "Proje başarıyla yapılandırıldı ve GitHub'a senkronize edildi."